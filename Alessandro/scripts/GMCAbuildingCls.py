...
#Variables of the GMCA
n_s          = config.getint("GMCA","n_s")
mints        = config.getfloat("GMCA","mints")
nmax         = config.getint("GMCA","nmax")
L0           = config.getint("GMCA","L0")
whitening    = config.getboolean("GMCA","whitening")
epsi         = config.getfloat("GMCA","epsi")
verbose      = config.getboolean("GMCA","verbose")
div          = config.getint("GMCA","div")
without_covx = config.getboolean("GMCA","without_covx")
try:
	AInit  = np.asarray(json.loads(config.get( "GMCA", "AInit")))
except:
	AInit=None
try:
	ColFixed  = np.asarray(json.loads(config.get( "GMCA", "ColFixed")))
except:
	ColFixed=None
...
parser.add_argument('--wtransform', action = 'store', dest = 'wtransform', default = wtransform, help = 'Type of wavelet transform.')
...
parser.add_argument('--whitening', action = 'store', dest = 'whitening', default =whitening , help = 'Whiten the data. I.e., the cov matrix of the observed data will be identity.')
parser.add_argument('--epsi', action = 'store', dest = 'epsi', default =epsi , help = 'Tolerance.')
