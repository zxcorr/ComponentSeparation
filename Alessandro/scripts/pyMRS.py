...
###################################################
## Modified by Alessandro Marins in january/2021 ##
...
#
def almtrans(map,lmax=None): #It get a map and return a tab from this map with: tab[i,j,k], where: i is l-multipoles, j is m-moment and k is 0 (real) or 1 (imaginary)
    ...
    alm = hpy.sphtfunc.map2alm(map,lmax=lmax) #alm[i]: i = l**2 + l + m + 1
...
#  ALM PRODUCT
#
#  multiply alm by low/high-filter
#  Loop is done in l-multipole 
...
def alm_product(tab,filt):
    ...
    for r in range(lmax):
        tab[r,:,:] = filt[r]*tab[r,:,:] #r <--> l

    return tab

# ALM2TAB : FROM VECTOR TO TAB
# alm --> table as [alm[l,m],real (0) or imaginary (1)]
# alm --> [:,:,0] = real(alm) e [:,:,1]= imag(alm)
# OBS: alm is a complex (continuos) vector

def alm2tab(alm,lmax):

    taille = np.size(alm)              # len(alm), alm = a[lm_0,alm_1,...alm_N], alm_i with i = l**2 + l + m + 1. The healpy function hp.Alm.getind get (lmax,l,m ) --> i; and hp.Alm.getlm (lmax,i) --> (l,m)
    tab = np.zeros((lmax+1,lmax+1,2))  # it will contain the two alm components real (tab[,,0]) and imaginary (tab[,,1]).

    for i in range(0,taille):
        l,m = hpy.sphtfunc.Alm.getlm(lmax,i)
        tab[l,m,0] = np.real(alm[i])
        tab[l,m,1] = np.imag(alm[i])

    return tab

# TAB2ALM : FROM TAB TO ALM
# TAB[:,:,0] = real(alm) e [:,:,1]= imag(alm) --> alm
# OBS: alm is a complex (continuos) vector
...
def spline2(size,l,lc): #size=lmax=3*nside  #l=2**(-j)*lc or 2**(-(j+1))*lc   #lc=1
    res = np.linspace(0,size,size+1) #res = [0,1,2,...,lmax]
    res = 2.0 * l * res / (lc *size)
    tab = (3.0/2.0)*(1.0/12.0 *( (abs(res-2))**3 - 4.0*(abs(res-1))**3 + 6*(abs(res))**3 - 4.0*( abs(res+1))**3+(abs(res+2))**3 ) )
    return tab

def compute_h(size,lc): #size=lmax=3*nside #lc --> 2**(-j)lc
    tab1 = spline2(size,2.*lc,1) 
    tab2 = spline2(size,1.*lc,1)
...
#
def wttrans(map,nscale=3+1,lmax=128): #nscale = J + 1 # map = X[i,:], a i-slice of the cube

    ech = 1 #2**0 # representa o fator lc=2**(j)=ech
    # i-map[:] --> i-tab[:,:,[0,1]] --> 
    taille = np.size(map) #npix

    alm = almtrans(map,lmax=lmax) #That is a tab alm values. It is a cube with [l,m,k], where k=0 (real) or 1 (imaginary)
    alm_temp = deepcopy(alm)      #it just copy the values and not its reference (pointer). Anyone modification in alm wont modify alm_temp

    LScale = deepcopy(map)        #it is a copy of the values from original maps (in a certain frequency bin)
    nside = hpy.get_nside(map)

    WT = np.zeros((taille,nscale)) #[WT] = npix X (J+1)

    for j in range(nscale-1): # j from 0 to J-1

        h = compute_h(lmax,ech)        # H filter in harmonic space
        #g = compute_g(lmax,ech)        # Needed if the difference is computed in the spherical harmonics domain (Carucci's comment)

        alm_temp = alm_product(alm,h)    # alm filtered by the H(igh) filter

        m = almrec(alm_temp,nside=nside) # alm_J --> map_rec

        #Lscale = c_(j)
        #m      = c_(j+1); j=0,...,J-1
        HScale = LScale - m # w_(j) = c_(j) - c_(j+1)   #wavelet coef
        LScale = m          # c_(j) = c_(j+1)           #scale coef

        WT[:,j] = HScale    # j in [0,J-1]

        ech = 2*ech

    WT[:,nscale-1] = LScale #c_J   #nscale = J+1 --> J = nscale - 1
